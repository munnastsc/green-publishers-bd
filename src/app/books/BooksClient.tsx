...
interface Props {
  ...
  authors: any[];
  publishers: any[];
  featuredPublisherId: number | null;
  featuredSectionEn: string;
  featuredSectionBn: string;
  otherSectionEn: string;
  otherSectionBn: string;
}

export default function BooksClient({
  books, categories, authors, publishers,
  featuredPublisherId, featuredSectionEn, featuredSectionBn,
  otherSectionEn, otherSectionBn
}: Props) {
  ...
  const filtered = books
    .filter((b: any) => {
      ...
      const matchPublisher = selPublisher ? b.publisherId === parseInt(selPublisher) : true;
  ...
  const featuredBooks = featuredPublisherId
    ? filtered.filter((b: any) => b.publisherId === featuredPublisherId)
    : [];
        ...
        {showFilters && (
          <div style={{
            ...
            <div className="input-group">
              ...
              <select className="form-control" value={selPublisher} onChange={e => setSelPublisher(e.target.value)}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <button
                onClick={() => { setSelCat(''); setSelAuthor(''); setSelPublisher(''); setPriceRange(2000); setSearch(''); }}
      ...
      {filtered.length === 0 ? (
        ...
        <>
          ...
          {featuredBooks.length > 0 && (
            <>
              <SectionHeading label={lang === 'en' ? featuredSectionEn : featuredSectionBn} color="var(--primary)" />
              <div className="books-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1.5rem' }}>
          ...
          {otherBooks.length > 0 && (
            <>
              <SectionHeading label={lang === 'en' ? otherSectionEn : otherSectionBn} color="#64748b" />
              <div className="books-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1.5rem' }}>
