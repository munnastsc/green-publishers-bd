...
export default async function Home() {
  ...
  try {
    ...
    if (p.banner && typeof p.banner.findMany === 'function') {
      banners = await p.banner.findMany({ where: { active: true }, orderBy: { createdAt: 'desc' } });
    }
  ...
  try {
    ...
    if (p.homeSection && typeof p.homeSection.findMany === 'function') {
      sections = await p.homeSection.findMany({ orderBy: { order: 'asc' } });
    }
    ...
    try {
      ...
      if (p.book && typeof p.book.findMany === 'function') {
        if (section.type === 'LATEST') {
          books = await p.book.findMany({ take: 12, orderBy: { createdAt: 'desc' }, include: { author: true, category: true, publisher: true } });
        } else if (section.type === 'CATEGORY' && section.targetId) {
          books = await p.book.findMany({ where: { categoryId: section.targetId }, take: 12, orderBy: { createdAt: 'desc' }, include: { author: true, category: true, publisher: true } });
        }
      }
    } catch (e) { console.error(`Section ${section.id} fetch failed:`, e); }
  ...
  try {
    const settingsRaw = await prisma.siteSetting.findMany({
      where: {
        key: {
          in: [
            'heroTitleEn', 'heroTitleBn', 'heroSubtitleEn', 'heroSubtitleBn', 'heroBtnEn', 'heroBtnBn',
            'feature1TitleEn', 'feature1TitleBn', 'feature1DescEn', 'feature1DescBn',
            'feature2TitleEn', 'feature2TitleBn', 'feature2DescEn', 'feature2DescBn',
          ]
        }
      }
    });
