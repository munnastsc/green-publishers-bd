...
import { BookMarked, Trash2, Pencil, X, Check, Plus } from 'lucide-react';

const BLANK = { titleEn: '', titleBn: '', descriptionEn: '', descriptionBn: '', imageUrl: '', fileUrl: '', sortOrder: '0' };

export default function AdminEducationalMaterialsPage() {
  ...
  const fetchItems = async () => {
    const res = await fetch('/api/educational-materials');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setEditingId(null);
      fetchItems();
      showMsg(editingId ? 'উপকরণ আপডেট হয়েছে!' : 'উপকরণ সংরক্ষিত হয়েছে!');
    }
    setLoading(false);
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('এই উপকরণটি মুছবেন?')) return;
    await fetch('/api/educational-materials', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
  ...
  const field = (key: keyof typeof BLANK, label: string, placeholder = '', type = 'text', span = false) => (
    <div className="input-group" style={span ? { gridColumn: '1 / -1' } : {}}>
  ...
  return (
    <div>
      <h2 style={{ fontSize: '1.8rem', marginBottom: '1.5rem', color: 'var(--primary-dark)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <BookMarked size={28} /> শিক্ষা উপকরণ পরিচালনা
      </h2>
      ...
      <div style={{ background: 'white', border: `2px solid ${editingId ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '10px', padding: '2rem', marginBottom: '2rem' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '1.25rem', color: editingId ? 'var(--primary)' : '#1e293b', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Plus size={18} /> {editingId ? 'উপকরণ সম্পাদনা করুন' : 'নতুন উপকরণ যোগ করুন'}
        </h3>
        <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div className="input-group">
            <label className="input-label">শিরোনাম (English) *</label>
            <input required type="text" className="form-control" placeholder="Introduction to Science" value={formData.titleEn} onChange={e => setFormData({ ...formData, titleEn: e.target.value })} />
          </div>
          <div className="input-group">
            <label className="input-label">শিরোনাম (বাংলা) *</label>
            <input required type="text" className="form-control" placeholder="বিজ্ঞানের পরিচয়" value={formData.titleBn} onChange={e => setFormData({ ...formData, titleBn: e.target.value })} />
          </div>
          ...
          <div className="input-group">
            ...
            <input type="url" className="form-control" placeholder="https://example.com/file.pdf" value={formData.fileUrl} onChange={e => setFormData({ ...formData, fileUrl: e.target.value })} />
          </div>
          <div className="input-group">
      ...
      <div style={{ background: 'white', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '2rem' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '1.5rem' }}>সকল উপকরণ ({items.length})</h3>
        {items.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>
            <BookMarked size={48} style={{ marginBottom: '1rem', opacity: 0.3 }} />
            <p>কোনো উপকরণ নেই। উপরে যোগ করুন।</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {items.map((item: any) => (
              <div key={item.id} style={{
                ...
                border: `1px solid ${editingId === item.id ? 'var(--primary)' : '#e2e8f0'}`,
                borderRadius: '8px',
                background: editingId === item.id ? '#f0f9ff' : 'white',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flex: 1, minWidth: '200px' }}>
                  {item.imageUrl ? (
                    ...
                    <div style={{ width: '44px', height: '44px', borderRadius: '8px', background: 'linear-gradient(135deg, #dbeafe, #93c5fd)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
