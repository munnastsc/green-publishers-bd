...
export default function AdminCategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setEditingId(null);
      fetchItems();
      showMsg(editingId ? 'Category updated!' : 'Category added!');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Delete this category? Books linked to it will lose their category.')) return;
  ...
  return (
    <div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '0.75rem' }}>
            <button type="submit" className="btn btn-blue" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Check size={16} /> {editingId ? 'Update Category' : 'Add Category'}
            </button>
            {editingId && (
      ...
      <div style={{ background: 'white', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '2rem' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '1rem' }}>All Categories ({items.length})</h3>
        {loading ? <p>Loading...</p> : items.length === 0 ? <p style={{ color: '#94a3b8' }}>No categories yet.</p> : (
