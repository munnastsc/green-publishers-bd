...
export default function AdminPublishersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/publishers', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    if (res.ok) {
      setFormData({ nameEn: '', nameBn: '' });
      setEditingId(null);
      fetchItems();
      showMsg(editingId ? 'Publisher updated!' : 'Publisher added!');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    await fetch('/api/publishers', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    fetchItems();
  };
  ...
  return (
    <div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '0.75rem' }}>
            <button type="submit" className="btn btn-blue" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Check size={16} /> {editingId ? 'Update Publisher' : 'Add Publisher'}
            </button>
            {editingId && (
      ...
      <div style={{ background: 'white', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '2rem' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '1rem' }}>All Publishers ({items.length})</h3>
        {loading ? <p>Loading...</p> : items.length === 0 ? <p style={{ color: '#94a3b8' }}>No publishers yet.</p> : (
